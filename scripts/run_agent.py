name: SWE-bench Pro Evaluation (Gemini)

on:
  workflow_dispatch:
    inputs:
      task_id:
        description: "Task ID to run"
        required: true
        default: "internetarchive__openlibrary-c4eebe6677acc4629cb541a98d5e91311444f5d4"

jobs:
  evaluate:
    runs-on: ubuntu-latest
    container:
      image: manojva/openlibrary-python312:latest
      options: --user root

    env:
      TASK_ID: ${{ github.event.inputs.task_id }}
      PYTHONPATH: /testbed
      GEMINI_API_KEY: ${{ secrets.GEMINI_API_KEY }}
      GEMINI_MODEL: gemini-1.5-pro

    steps:
      - uses: actions/checkout@v4

      - name: Prepare directories
        run: mkdir -p /workspace/artifacts /testbed

      - name: Clone OpenLibrary
        shell: bash
        run: |
          COMMIT_HASH=$(echo "$TASK_ID" | rev | cut -d'-' -f1 | rev)
          git clone https://github.com/internetarchive/openlibrary.git /testbed
          cd /testbed
          git checkout "$COMMIT_HASH"

      - name: Configure Git
        run: |
          git config --global user.email "agent@example.com"
          git config --global user.name "SWE Agent"

      - name: Install system dependencies
        run: |
          apt-get update && apt-get install -y build-essential libpq-dev

      - name: Install Python dependencies
        run: |
          python -m pip install --upgrade pip
          pip install --no-cache-dir \
            pytest pytest-asyncio pytest-rerunfailures \
            statsd pymarc \
            web.py simplejson babel python-dateutil requests \
            psycopg2-binary Cython setuptools wheel \
            google-generativeai google-genai \
            python-memcached validate_email lxml eventer \
            pymemcache aiofiles luqum

      - name: Install Infogami
        run: |
          cd /opt
          if [ ! -d infogami ]; then git clone https://github.com/internetarchive/infogami.git; fi
          pip install -e infogami

      - name: Pre-validation
        run: |
          cd /testbed
          pytest --maxfail=5 --disable-warnings --tb=short | tee /workspace/artifacts/pre_validation.log

      - name: Run Gemini agent
        run: |
          python /testbed/scripts/run_agent.py \
            --task-id "$TASK_ID" \
            --repo-path /testbed \
            --log-path /workspace/artifacts/agent.log \
            --promp
