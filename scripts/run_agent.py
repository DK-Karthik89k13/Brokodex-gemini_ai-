...
def log_event(payload):
    """Log an event as JSONL with timestamp"""
...
def safe_import(module_name, package_name=None):
    """Import module, install automatically if missing"""
...
def run_pytest(test_target, repo, log_path, stage):
    ...
    (Path(log_path).parent / f"{Path(log_path).stem}.exit").write_text(str(result.returncode))
    log_event({
        "stage": stage,
...
def main():
    ...
    if args.task_file and "def find_staged_or_pending" not in content:
